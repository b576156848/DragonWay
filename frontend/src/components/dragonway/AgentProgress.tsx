...
const AgentProgress = ({ onComplete, currentStepId, status }: AgentProgressProps) => {
  const [currentStep, setCurrentStep] = useState(0);
  const [completed, setCompleted] = useState<number[]>([]);
  const isControlled = Boolean(currentStepId);

  useEffect(() => {
    if (isControlled) {
      const activeIndex = Math.max(AGENT_STEPS.findIndex(step => step.id === currentStepId), 0);
      setCurrentStep(activeIndex);
      setCompleted(AGENT_STEPS.map((_, index) => index).filter(index => index < activeIndex || status === 'ready'));
      return;
    }
  ...
  }, [currentStep, currentStepId, isControlled, onComplete, status]);
    ...
    <div className="max-w-xl mx-auto py-20">
      ...
      >
        <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-primary/20 bg-primary/5 mb-4">
          ...
          <span className="text-sm text-primary font-medium">Agent Running</span>
        </div>
        <h2 className="text-2xl font-bold text-foreground mb-2">Analyzing your product</h2>
      ...
      <div className="glass-card p-6">
            ...
            return (
              ...
              >
                ...
                <span className={`text-sm font-medium font-mono ${
                  isCompleted ? 'text-primary/80' : isActive ? 'text-foreground' : 'text-muted-foreground/50'
                }`}>
                  {step.label}
                </span>
                {isActive && (
        ...
        <div className="mt-6 h-1 bg-secondary rounded-full overflow-hidden">
          <motion.div
            ...
            style={{ background: 'linear-gradient(90deg, hsl(174 72% 52%), hsl(196 80% 46%))' }}
            initial={{ width: '0%' }}
