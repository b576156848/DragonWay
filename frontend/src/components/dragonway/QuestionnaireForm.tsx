...
const QuestionnaireForm = ({ formData, onChange, onSubmit, isSubmitting = false }: QuestionnaireFormProps) => {
  ...
  return (
    ...
    >
      ...
      <div className="flex justify-between mt-8">
        ...
        {isLastStep ? (
          <Button onClick={onSubmit} className="bg-primary text-primary-foreground hover:bg-primary/90 px-8" disabled={isSubmitting}>
            {isSubmitting ? 'Starting...' : 'Start Matching'}
            <ArrowRight className="w-4 h-4 ml-2" />
          </Button>
        ) : (
          <Button onClick={() => setStep(s => s + 1)} variant="ghost" className="text-primary hover:text-primary/80">
            Next
